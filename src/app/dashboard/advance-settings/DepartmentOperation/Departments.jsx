...
export const Departments = () => {
  const savedDepartmentName = localStorage.getItem("ticket_defaultDepartment");
      ...
      <div className="overflow-y-auto overflow-hidden h-[16em]">
        ...
        {responseData.data?.success &&
          responseData.data.data?.map((item) => (
            <button
              key={item.id}
              className="flex group mb-2 w-full soft-bg hover:bg-gray-900 text-white items-center justify-between px-3 py-2 text-xs font-medium transition-colors duration-300 transform rounded-lg   dark:text-gray-200"
            >
              <div className="flex items-center gap-x-2 ">
                <span className="w-2 h-2 rounded-full bg-slate-500"></span>
                <span>{item.name}</span>
                {defaultDepartment === item.id && (
                  <span className="bg-gray-600 rounded-full px-2">Default</span>
                )}
              </div>
              <div className=" flex gap-2 items-center transition-all">
                <button
                  onClick={() => handleDefaultDepartment(item.id)}
