  ...
  const displayNotes = (notes) => {
    ...
    for (let noteIndex = 0; noteIndex < notesArray.length; noteIndex++) {
      ...
      if (noteLength === noteIndex + 1) {
        ...
        newNotesArray.push(<Note key={noteObject.id} note={noteObject} />);
      }
    }
  ...
  return (
    <>
      {notesResponse.isError && (
        <SimpleErrorMessage message={notesResponse.data?.data?.message} />
      )}
...
export const Note = ({ note }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => {
      return axios.delete(`/api/tickets/${ticketData.id}/notes/${note.id}`);
    },
    onSuccess: () => {
        ...
        >
          {deleteMutation.isLoading ? (
            <div className="absolute">
              <VscLoading size={45} color="white" className="animate-spin" />
            </div>
          ) : (
