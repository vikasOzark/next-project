            ...
            <form onSubmit={handleSubmit}>
                <div className="">
                    <div>
                        ...
                        />
                        {departmentMutation.isSuccess && !departmentMutation.data.success && <small className='text-red-500 font-bold'>{departmentMutation.data?.data.department}</small>}
                        </div>
                    </div>

                    <div>
                        <label htmlFor="desprition" className="block text-sm font-medium leading-6 text-gray-900">
                        Short Description
                        </label>
                        <div className="mt-2 rounded-md shadow-sm">
                        <textarea
                ...
                <div className=" py-3 sm:flex sm:flex-row-reverse">
                    {!departmentMutation.isLoading && 
                        <SubmitButton title={"Create department"} cssClass={"text-white"} icon={<VscOrganization />} />
                    }
                    {departmentMutation.isLoading &&
