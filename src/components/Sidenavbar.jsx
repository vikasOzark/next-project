...
import { CursorArrowRaysIcon, HomeIcon, LifebuoyIcon, PlusCircleIcon } from "@heroicons/react/20/solid";
          ...
          <div className="flex flex-col justify-between flex-1 mt-6">
            <nav className="-mx-3 space-y-3 ">
              {dashboardLinks.map((item) => (
                <NavigationLink key={item.title} href={item.route} text={item.title} icon={item.icon} />
              ))}
            </nav>
            ...
            <div className="mb-3">
              <div className="">
                <button
                  onClick={() => signOut()}
                  className="bg-gray-300 flex items-center gap-3 justify-center px-4 py-1 w-full mb-2 hover:bg-gray-400 transition-all rounded font-bold "
                >
                  <VscSignOut />
                  Sign out
                </button>
              ...
              <div className="flex items-center justify-between">
                ...
                >
                  <PlusCircleIcon className="h-5 w-5" title="create new department" />
                </button>
              </div>
